...
export async function buildCreateAdminCellTx(
    ...
    console.log('builder from address', txSenderAddress);
    ...
    const rcData = SerializeRCData({
        type: 'RCRule',
        value: { smt_root: new Reader(options.auth_smt_root).toArrayBuffer(), flags: 2 },
    });
    ...
    txSkeleton = await completeTx(provider, txSkeleton, txSenderAddress);
...
async function completeTx(
    ...
    txSkeleton = await common.payFeeByFeeRate(txSkeleton, [fromAddress], feeRate, undefined, {config: provider.config});
    return txSkeleton;
}
