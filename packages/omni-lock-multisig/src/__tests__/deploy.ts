...
import { CkitConfig, CkitProvider, predefined } from "@ckitjs/ckit";
...
export async function loadSecp256k1ScriptDep(
  ...
  const typeScript = nonNullable(
    nonNullable(genesisBlock.transactions[0]).outputs[1]
  ).type;
...
export async function deployCkbScripts(
  ...
  return deployScripts(provider, [sudtBin, acpBin, rcBin], ckbPrivateKey);
}
