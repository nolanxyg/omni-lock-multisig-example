...
import {CkitConfig, CkitConfigKeys, CkitProvider, TransferCkbBuilder, internal} from '@ckitjs/ckit';
...
export class TestProvider extends CkitProvider {
    ...
    getGenesisSigner(index: number): EntrySigner {
        return new internal.Secp256k1Signer(nonNullable(this.testPrivateKeys[index]), this, this.newScript('SECP256K1_BLAKE160'));
    }
    ...
    private async deployDeps(): Promise<CkitConfig> {
        ...
        const scripts = await deployCkbScripts(appRootPath.resolve('/deps'), this, this.#_assemberPrivateKey);
    ...
    generateAcpSigner(key: CkitConfigKeys = 'ANYONE_CAN_PAY'): EntrySigner {
        return new internal.Secp256k1Signer(randomHexString(64), this, this.newScript(key));
    }
